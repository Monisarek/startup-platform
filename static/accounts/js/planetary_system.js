document.addEventListener('DOMContentLoaded', function () {
  ...
  try {
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      if (solarSystem && mouseCoords) {
        // Check if solarSystem and mouseCoords exist
        const rect = solarSystem.getBoundingClientRect()
        mouseCoords.style.left = `${Math.min(rect.width - 80, Math.max(0, e.clientX - rect.left))}px`
        mouseCoords.style.top = `${Math.min(rect.height - 30, Math.max(0, e.clientY - rect.top))}px`
        mouseCoords.textContent = `Mouse: (${Math.round(e.clientX - rect.left)}, ${Math.round(e.clientY - rect.top)})`
    ...
    function checkInactivity() {
      const now = Date.now()
      if (
        now - lastInteractionTime >= inactivityTimeout &&
        !isReturningToCenter &&
        !isPaused
      ) {
    ...
    const planetObjects = []
    planets.forEach((planet_element, index) => {
      // Renamed planet to planet_element
      ...
      if (!orbit || !planetOrientation) {
        console.error(
          'Could not find orbit or planetOrientation for',
          planet_element
        )
        return
      }

      const orbitSizeStyle =
        getComputedStyle(orbit).getPropertyValue('--orbit-size')
      const orbitTimeStyle =
        getComputedStyle(orbit).getPropertyValue('--orbit-time')
      ...
      planetObjects.push({
        ...
        orbit: orbit,
        size: parseFloat(
          getComputedStyle(planet_element).getPropertyValue('--planet-size')
        ),
        orbitSize: orbitSize,
        orbitTime: orbitTime,
        angle: initialAngle,
    ...
    function updatePlanets() {
      ...
      planetObjects.forEach((planetObj) => {
        try {
          ...
          const progress =
            (elapsedSeconds % orbitTimeSeconds) / orbitTimeSeconds
          ...
          console.error(
            'Error updating planet position:',
            error,
            'for planetObj',
            planetObj
          )
    ...
    if (galaxySelector && galaxyList) {
      ...
      if (totalItems > 0) {
        // Ensure itemsArray is not empty
      ...
      function selectGalaxy(galaxyName) {
        ...
        document
          .querySelectorAll(`.galaxy-item[data-name="${galaxyName}"]`)
          .forEach((item) => {
        ...
        if (config.urls && config.urls.planetarySystemBase) {
          ...
          console.error(
            'Planetary system base URL not configured in config.urls.planetarySystemBase'
          )
        }
      }
      ...
      function smoothScroll() {
        ...
        if (timeSinceLastScroll < 100) {
          ...
          scrollVelocity *= 0.9
          if (totalItems > 0) {
            // Ensure itemsArray and totalItems are valid
            ...
            if (totalWidth > 0) {
              ...
              if (scrollLeftVal >= totalWidth) {
                ...
                scrollLeftVal <= 0 &&
                galaxySelector.scrollWidth > galaxySelector.clientWidth
              ) {
                // Add check for scrollWidth > clientWidth
      ...
      if (galaxyListItems.length > 0) {
        // Only start if there are items
        requestAnimationFrame(smoothScroll)
      }
    ...
    if (crosshairCoords && offsetX !== undefined && offsetY !== undefined) {
      // Check elements and variables
      crosshairCoords.textContent = `Center: (${Math.round(offsetX)}, ${Math.round(offsetY)}, 0)`
    }
    ...
    if (savedGalaxy && savedGalaxy !== initialGalaxy) {
      ...
      if (config.urls && config.urls.planetarySystemBase) {
        ...
        console.error(
          'Planetary system base URL not configured for redirection.'
        )
      }
    } else {
      if (galaxySelector) {
        // Ensure galaxySelector exists
        const selectedItems = galaxySelector.querySelectorAll(
    ...
    if (galaxySelector) {
      // Ensure galaxySelector exists for scroll event
      galaxySelector.addEventListener('scroll', () => {
        ...
        if (selectedItems.length > 0) {
          ...
          const distanceFromCenter = Math.abs(containerCenter - itemCenter)
          if (
            distanceFromCenter > containerRect.width / 4 &&
    ...
    if (planets.length > 0) {
      // Only start if there are planets
      requestAnimationFrame(updatePlanets)
    }
  } catch (error) {
